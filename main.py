...
#number of getting tweets
params = {'count': 1}

#hold last tweet folder
file_name = 'final_twi.txt'

#get basic data from basic data folder
...
#        logging.debug(timeline)
        for tweet in timeline:
            ...
            if content != tw_txt:
                ...
                #TODO check distance between here and where?
                if tw_txt.find('緊急地震速報') != -1 and info_where != None and info_level != None:
                    ...
                    for loop in prefec:
                        if loop in str(info_where.group(1)):
                            info_prefec = loop
                            print('{}での地震を観測'.format(info_prefec))
                        else:
                            info_prefec = ''
                    logging.debug(info_prefec)
                    logging.debug(where['都道府県'])
